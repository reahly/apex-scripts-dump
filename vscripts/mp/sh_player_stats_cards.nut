...
void function StatCard_ConstructRankedBadges( var panel, entity player, string rankedPeriodRef )
{
	...
	if ( RankedPeriod_HasSplits( rankedPeriodItemFlavor )  )
	{
		if ( rankedPeriodRef == GetCurrentStatRankedPeriodRefOrNull() && RankedPeriod_IsFirstSplitActive( rankedPeriodItemFlavor ) )
			StatsCard_ConstructRankBadgesForSingleBadge( rui, player, rankedPeriodRef )
		else
			StatsCard_ConstructRankBadgesForDoubleBadge( rui, player, rankedPeriodRef  )
	}
	else
	{
		StatsCard_ConstructRankBadgesForSingleBadge( rui, player, rankedPeriodRef )
...
void function StatsCard_ConstructRankBadgesForDoubleBadge( var rui, entity player,  string rankedPeriodRef  )
{
	...
	RankedDivisionData firstSplitDivData = Ranked_GetHistoricalRankedDivisionFromScore( firstSplitScore, rankedPeriodRef )
	...
	RankedDivisionData secondSplitDivData = Ranked_GetHistoricalRankedDivisionFromScore( secondSplitSplitScore, rankedPeriodRef )

	if ( rankedPeriodRef == GetCurrentStatRankedPeriodRefOrNull() )
	{
		PopulateRuiWithRankedBadgeDetails( secondSplitBadgeRui, secondSplitSplitScore, Ranked_GetLadderPosition( GetUIPlayer() ) )
	}
	else
	{
		int historicalLadderPosition = Ranked_GetHistoricalLadderPosition( player, rankedPeriodRef, false  )
...
void function ConstructWeaponStatStructArray( entity player )
{
	...
	foreach( ItemFlavor weapon in GetAllWeaponItemFlavors() )
	{
		...
		newWeapon.portrait = WeaponItemFlavor_GetHudIcon( weapon )
