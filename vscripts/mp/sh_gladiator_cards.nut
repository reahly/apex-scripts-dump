...
void function OnItemFlavorRegistered_Character( ItemFlavor characterClass )
{
	...
	//
	{
		...
		{
			Assert( !( frame in fileLevel.frameCharacterMap ) || ItemFlavor_GetHumanReadableRef( frame ) == "gcard_frame__temp" )
	...
	//
	{
		...
		{
			Assert( !( stance in fileLevel.stanceCharacterMap ) || ItemFlavor_GetHumanReadableRef( stance ) == "gcard_stance__temp" )
	...
	foreach ( int index, ItemFlavor badge in badgeList )
	{
		...
		if ( GladiatorCardBadge_IsCharacterBadge( badge ) )
		{
			Assert( !( badge in fileLevel.badgeCharacterMap ) )
...
//
	...
	foreach ( int index, ItemFlavor tracker in trackerList )
	{
		...
		Assert( !(tracker in fileLevel.trackerCharacterMap) || GladiatorCardTracker_IsTheEmpty( tracker ) )
...
ItemFlavor ornull function GladiatorCardBadge_GetCharacterFlavor( ItemFlavor flavor )
{
	...
	if ( flavor in fileLevel.badgeCharacterMap )
		return fileLevel.badgeCharacterMap[flavor]

	Assert( !GladiatorCardBadge_IsCharacterBadge( flavor ) )
	return null
}
#endif
