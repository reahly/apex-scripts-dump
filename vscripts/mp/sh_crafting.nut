...
void function Crafting_RegisterNetworking()
{
	...
	Remote_RegisterClientFunction( "ServerCallback_CL_MaterialsChanged", "int", -1, INT_MAX, "int", -1, INT_MAX )
...
bool function Crafting_Harvester_IsNotBusy( entity player, entity ent )
{
...
void function ServerCallback_CL_MaterialsChanged( int amount, int difference )
{
	if ( file.fullmapRui.len() != 0 && file.fullmapRui[0] != null )
		RuiSetInt( file.fullmapRui[0], "craftingMaterials", amount )

	if ( difference > 0 )
		AnnouncementMessageRight( GetLocalClientPlayer(), Localize( "#CRAFTING_HARVESTER_AWARD", difference ), "", <214,214,214>, $"", 2, "" )
	else
		AnnouncementMessageRight( GetLocalClientPlayer(), Localize( "#CRAFTING_HARVESTER_BALANCE_UPDATE", amount ), "", <214,214,214>, $"", 2, "" )
}
...
bool function Crafting_Workbench_IsNotBusy( entity player, entity ent )
{
...
void function Crafting_OnWorkbenchMenuClosed( )
{
	if ( !file.isEnabled)
		return
...
void function Crafting_PopulateItemRuiAtIndex( var rui, int index )
{
	...
	if ( validItemList.len() != 0 && item.category != "evo" )
	{
		...
		else if ( lootRef.lootType == eLootType.AMMO && validItemList.len() > 1 && validItemList[1] != "" )
		{
			...
			RuiSetInt( rui, "ammoAmount", lootRef.countPerDrop )
			RuiSetInt( rui, "ammoAmountAlt", lootRefAlt.countPerDrop )
		}
