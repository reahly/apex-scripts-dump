...
void function Clubs_Init()
{
	...
	//
		...
		file.errorCodeMap[CLUB_ERROR_CODE_KICK_COOL_OFF] <- "#CLUB_OP_FAIL_JOIN_COOL_OFF"
		file.errorCodeMap[CLUB_ERROR_CODE_JOIN_COOL_OFF] <- "#CLUB_OP_FAIL_JOIN_COOL_OFF"
		file.errorCodeMap[CLUB_ERROR_CODE_HOP_COOL_OFF] <- "#CLUB_OP_FAIL_JOIN_COOL_OFF"
...
bool function Clubs_DoesMeetJoinRequirements( ClubHeader clubHeader )
{
	...
	RankedTierData highestRankedTier = Ranked_GetHighestHistoricalRankedTierData( GetLocalClientPlayer() )
...
ItemFlavor function GetRandomClubLogoElementFlavorByType( int elementType )
{
...
ClubLogo function GenerateRandomClubLogo()
{
	ClubLogo newLogo

	array<vector> colorSet = ClubLogo_GetRandomDoubleSplitComplementaryColorSet()
	...
	for ( int i = 0; i < CLUB_LOGO_LAYER_MAX; i++ )
	{
		...
		newLayer.elementFlav = GetRandomClubLogoElementFlavorByType( i )
		//
		//
...
vector function ClubLogo_GetRandomLogoColor()
{
	...
	int randomRow = RandomIntRange( 0, rowCount )
...
array<vector> function ClubLogo_GetRandomDoubleSplitComplementaryColorSet()
{
	vector startingColor = ClubLogo_GetRandomLogoColor()
...
string function ClubLogo_ConvertLogoToString( ClubLogo logo )
{
	...
	printf( "ClubLogoDebug: ConvertLogoToString(): Converted logo to string: %s", clubLogoString )
...
{
	printf( "ClubSearchTagDebug: CreateSearchTagBitmask: creating mask for %i tags", searchTags.len() )
	...
	foreach ( ItemFlavor searchTag in searchTags )
	{
		...
		printf( "ClubSearchTagDebug: CreateSearchTagBitmask: Adding flag %i", flag )
	...
	printf( "ClubSearchTagDebug: CreateSearchTagBitmask: returning %i", searchTagBitMask )
...
{
	printf( "ClubSearchTagDebug: %s(): Adding a search tag", FUNC_NAME() )
	Assert( ItemFlavor_GetType( searchTag ) == eItemType.club_search_tag )
...
{
	printf( "ClubSearchTagDebug: %s(): Removing a search tag", FUNC_NAME() )
	Assert( ItemFlavor_GetType( searchTag ) == eItemType.club_search_tag )
...
{
	printf( "ClubSearchTagDebug: %s(): returning %i search tags", FUNC_NAME(), file.selectedSearchTags.len() )
	return file.selectedSearchTags
}

void function ClubSearchTag_ClearSelectedSearchTags()
{
	printf( "ClubSearchTagDebug: %s()", FUNC_NAME() )
...
void function ServerToUI_AddPlayerDataForPlacementReport( int placement )
{
	...
	printf( "ClubEventDebug: %s(): Added Player Data for squad placement Event", FUNC_NAME() )
	Clubs_ReportMatchPlacementToClub()
}
#endif
...
void function Clubs_ReportMatchPlacementToClub()
{
	...
	if ( file.postMatchSummaryData.squadplacement > MAX_PLACEMENT_FOR_CLUB_EVENT )
		return
	...
	foreach ( ClubSquadSummaryPlayerData summaryData in file.postMatchSummaryData.playerData )
	{
		if ( !Clubs_IsUserAClubmate( summaryData.nucleusID ) )
		{
			printf( "ClubEventDebug: %s(): Removed a player from Club Report as they are not a clubmate", FUNC_NAME() )
			file.postMatchSummaryData.playerData.removebyvalue( summaryData )
		}
	}
	if ( file.postMatchSummaryData.playerData.len() == 0 )
		return

	file.postMatchSummaryData.playerData.sort( SortClubMembersByPerformance )

	printf( "ClubEventDebug: %s(): Total clubmates for event: %i", FUNC_NAME(), file.postMatchSummaryData.playerData.len() )

	if ( file.postMatchSummaryData.playerData[0].nucleusID != GetLocalClientPlayer().GetPINNucleusId() )
	{
		printf( "ClubEventDebug: %s(): Terminating function (player is not best performing out of club squad)", FUNC_NAME() )
		return
	}

	array<string> playerIDs
	for ( int i = 0; i < file.postMatchSummaryData.playerData.len(); i++ )
	{
		playerIDs.append( file.postMatchSummaryData.playerData[i].nucleusID )
	}
	string compiledPlayerNames = CompilePlacementNamesIntoOneString( playerIDs )
	string playlistName = GetCurrentPlaylistVarInt( "maskPlaylistNameForClubReport", 0 ) == 0 ? Localize( GetCurrentPlaylistVarString( "name", "#PL_TRIO" ) ) : Localize( "#PL_TRIO" )

	string eventText = compiledPlayerNames + CLUB_EVENT_DELIMITER + playlistName

	printf( "ClubEventDebug: %s(): squadPlacement = %i, eventText = %s", FUNC_NAME(), file.postMatchSummaryData.squadplacement, eventText )
	ClubPublishEvent( CLUB_EVENT_PROGRESS, file.postMatchSummaryData.squadplacement, eventText )

	file.postMatchSummaryData.playerData.clear()
	file.postMatchSummaryData.squadplacement = -1
}
#endif
...
string function CompilePlacementNamesIntoOneString( array<string> playerIDs )
{
	...
	{
		string delimiter
		int namesLeft = (playerIDs.len() - 1) - i
		if ( namesLeft > 1 )
...
#if UI && DEV 
void function DEV_ReportFakePlacementEvent( int forceSquadSize = -1 )
{
	if ( !ClubIsValid() )
		return
	...
	printf( "PlacementEventDebug: %s(): Reporting %i players placing in an event", FUNC_NAME(), playerIDs.len() )
...
void function Clubs_CheckClubPersistenceThread()
{
	...
	if ( IsLobby() )
	{
		...
		{
			if ( myClubHeader.clubID != "" && persistentClubID != PENDING_CLUB_REQUEST )
			{
				Remote_ServerCallFunction( "ClientCallback_SetClubID", myClubHeader.clubID )
			}
		}
	}
}
...
void function Clubs_SetClubTabUserCount()
{
	...
	foreach ( panel in panels )
	{
		if ( GetPanelTabTitle( panel ) == "#LOBBY_CLUBS" )
		{
			...
			printf( "Clubs_SetClubTabUserCount(): Found club index: %i", clubPanelIndex )
		}
	}
	if ( clubPanelIndex == -1 )
		return
...
void function Clubs_MonitorCrossplayChangeThread()
{
	...
	while ( ClubIsValid() && userCrossplayEnabled == clubCrossplayEnabled )
	{
