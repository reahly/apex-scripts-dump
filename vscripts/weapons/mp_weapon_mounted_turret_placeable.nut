...
const string MOUNTED_TURRET_PLACEABLE_WEAPON_NAME = "mp_weapon_mounted_turret_placeable"
...
bool function MountedTurretPlaceable_CanUse( entity player, entity ent )
{
...
{
	return ( turret in file.turretEligibleForRefund && file.turretEligibleForRefund[ turret ] && turret.GetHealth() == turret.GetMaxHealth() )
}
	...
	string function MountedTurretPlaceable_UseTextOverride( entity ent )
	{
		...
		else if ( !MountedTurretPlaceable_CanUse( player, ent ) || player.IsTitan() || GradeFlagsHas( ent, eGradeFlags.IS_BUSY ) )
		{
