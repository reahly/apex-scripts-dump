...
const float MORTAR_RING_BOMB_AIRBURST_HEIGHT = 750.0
...
CrosshairTargetData function GetCrosshairTargetData( entity player )
{
	...
	TraceResults crosshairResults = TraceLineHighDetail( data.crosshairStart, crosshairEnd, [ player ], (TRACE_MASK_SHOT | CONTENTS_BLOCKLOS ) & ~CONTENTS_WINDOW, TRACE_COLLISION_GROUP_NONE )
	...
	if( flattenedDistanceToTarget < MORTAR_MIN_FIRE_DISTANCE )
	{
		...
		TraceResults downTraceResults = TraceLineHighDetail( data.groundTarget, downTraceEnd, [ player ], (TRACE_MASK_SHOT | CONTENTS_BLOCKLOS ) & ~CONTENTS_WINDOW, TRACE_COLLISION_GROUP_NONE )
