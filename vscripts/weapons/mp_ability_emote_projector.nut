...
global function GetDataIntForEmoteIcon
#endif
...
const int EMOTE_DATAINT_INDEX = 1
const int EMOTE_PROJECTOR_LIMIT = 1
...
void function OnWaypointCreated( entity wp )
{
	...
	if ( wpType == eWaypoint.EMOTE_ICON )
	{
		...
		thread ClientSideEmoteIconThink( wp, guid )
	}
	else if ( wpType == eWaypoint.EMOTE_ICON_TEXT )
	{
...
void function ClientSideEmoteIconThink( entity wp, int guid )
{
	...
	while ( true )
	{
		...
		if ( !active )
		{
			if ( isInView )
			{
				...
				{
					int dataInt = wp.GetWaypointInt( EMOTE_DATAINT_INDEX )
					thread CreateClientSideEmoteIcon( wp, guid, wp.WaypointGetCreationTime(), dataInt )
					active = true
				}
			}
		}
...
void function CreateClientSideEmoteIcon( entity wp, int guid, float startTime, int dataInt, bool lastsForever = false )
{
	wp.Signal( "CreateClientSideEmoteIcon" )
...
int function GetDataIntForEmoteIcon( entity player, ItemFlavor item )
{
	return 0
}
...
void function TrackFocusedHolospray( entity player )
{
	...
	while ( IsAlive( player ) )
	{
		...
		if ( GetFocusedWaypointEnt() != null && player.GetUsePromptEntity() == null )
		{
			...
			if ( wp.GetWaypointType() == eWaypoint.EMOTE_ICON_TEXT )
			{
