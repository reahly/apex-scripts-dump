	...
	while( true )
	{
		...
		if ( IsValid( file.cameraRui ) )
		{
			...
			if ( IsValid( trace.hitEnt ) )
			{
				...
				else if ( (IsVaultPanel( trace.hitEnt ) || IsVaultPanel( parentEnt )) && player.GetPlayerNetBool( "hasDataKnife" ) )
				{
					targetString = "#HINT_VAULT_USE"
				}
				...
				else if ( IsRespawnBeacon( trace.hitEnt ) && TeammatesWaitingToBeRespawned( player.GetTeam() ) )
				{
...
float function GetNeurolinkRange( entity player )
{
	...
	return file.neurolinkRange + additional
}
#endif
