...
//
const int TESLA_TRAP_LINK_DAMAGE_AMOUNT = 15
...
function MpWeaponTeslaTrap_Init()
{
	...
	PrecacheModel( TESLA_TRAP_TRIGGER_RADIUS_MODEL )
		...
		AddCallback_MinimapEntShouldCreateCheck( DontCreateRuisForEnemyTraps )
	#endif //
}
...
TeslaTrapPlacementInfo function TeslaTrap_GetPlacementInfo( entity player, entity proxy, bool ignorePlacedTraps = false, int maxFallbacks = 3 )
{
	...
	TraceResults viewTraceResults = TraceLine( eyePos, eyePos + player.GetViewVector() * (TESLA_TRAP_PLACEMENT_RANGE_MAX * 2), ignoreEnts, TRACE_MASK_SOLID, TRACE_COLLISION_GROUP_NONE, player )
	...
	TraceResults downResults = TraceHull( fwdResults.endPos, fwdResults.endPos - TESLA_TRAP_PLACEMENT_TRACE_OFFSET, TESLA_TRAP_BOUND_MINS, TESLA_TRAP_BOUND_MAXS, ignoreEnts, TRACE_MASK_SOLID, TRACE_COLLISION_GROUP_NONE, <0, 0, 1>, player )
...
bool function TeslaTrap_PlayerReachPos( entity player, vector pos, bool doTrace, float degrees, array<entity> ignoreEnts )
{
	...
	if ( doTrace )
	{
		TraceResults trace = TraceLine( player.EyePosition(), pos, ignoreEnts, TRACE_MASK_SOLID, TRACE_COLLISION_GROUP_NONE, player )
...
#if(CLIENT)
bool function DontCreateRuisForEnemyTraps( entity ent )
{
	if ( ent.GetScriptName() != TESLA_TRAP_NAME )
		return true
