...
void function SetUpNewPage( int pageIndex )
{
	...
	foreach ( int slot, SpeakerLine line in page.speakers )
	{
		...
		{
			SetCharacterInfoForSlot( rui, slot, line.character )
	...
	SetNarratorInfo( rui, page.narratorName )
...
void function OnClickedToAdvance( bool isForward )
{
	...
	if ( (Time() - s_lastScreenOpenTime) < SCREEN_OPEN_LOCKOUT_TIME )
		return
...
void function SetCharacterInfoForSlot( var rui, int slot, string speakerName )
{
	asset portraitButtonImage = $""
	string portraitButtonText = ""
	bool isNPC = false
	...
	switch ( slot )
	{
		case eSlot.TOP:
			...
			RuiSetImage( rui, "topPortraitButtonImage", info.portraitImage )
		...
		case eSlot.BOTTOM:
			...
			RuiSetImage( rui, "bottomPortraitButtonImage", info.portraitImage )
...
void function SetNarratorInfo( var rui, string narratorName )
{
	if ( narratorName == "" )
	{
	...
	RuiSetImage( rui, "narratorPortraitImage", info.portraitImage )
