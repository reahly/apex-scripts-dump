...
void function InitSystemPanel( var panel )
{
	...
	file.suicideButtonData[ panel ] <- clone data
	...
	file.abandonMissionButtonData[ panel ].label = "#QUEST_LEAVE_MATCH"
	...
	file.suicideButtonData[ panel ].label = "#BUTTON_SUICIDE"
	file.suicideButtonData[ panel ].activateFunc = TryRespawnAndChangeCharacters
...
void function UpdateSystemPanel( var panel )
{
	...
	if ( IsConnected() && !IsLobby() )
	{
		...
		{
			if ( IsPVEMode() )
				SetButtonData( panel, buttonIndex++, file.abandonMissionButtonData[ panel ] )
