...
void function PreviewWeaponSkin( ItemFlavor weaponSkinFlavor )
{
		...
		if ( InputIsButtonDown( KEY_LSHIFT ) )
			printt( GetGlobalSettingsString( weaponSkinFlavor._____INTERNAL_settingsAsset, "grxRef" ) )
	#endif //
...
bool function ShouldDisplayWeaponSkin( ItemFlavor weaponSkin )
{
	...
	{
		if ( !IsItemFlavorUnlockedForLoadoutSlot( LocalClientEHI(), Loadout_Character(), weaponSkin ) )
			return false
	}
