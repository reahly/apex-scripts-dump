...
void function PrivateMatch_TeamRoster_Configure( PlacementStruct teamPlacement, string teamName, array<PrivateMatchStatsStruct> playerTeamStatsArray )
{
	int placement = -1
...
void function PrivateMatch_TeamPlacement_Update( table< int, array< PrivateMatchStatsStruct > > teamPlacementMap )
{
	...
	foreach ( teamIndex, teamPlacement in file.teamPlacement )
	{
		...
		PrivateMatch_TeamRoster_Configure( teamPlacement, "ASDF", playerTeamStatsArray )

		HudElem_SetRuiArg( teamPlacement.headerPanel, "teamName", GameRules_GetTeamName( teamPlacement.teamIndex ) )
...
void function SetTeamName_OnOpen()
{
	...
	Hud_SetUTF8Text( textEntry, file.setTeamNameDialog_teamName )
...
void function SaveButton_OnClick( var button )
{
	...
	string newTeamName = Hud_GetUTF8Text( textEntry )
