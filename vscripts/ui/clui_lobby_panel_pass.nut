...
void function BattlePass_PopulateRewardButton( BattlePassReward bpReward, var rewardButton, bool isOwned, bool canUseTallButton, var ruiOverride = null )
{
	...
	if ( ItemFlavor_GetType( bpReward.flav ) == eItemType.voucher )
	{
		int points = Voucher_GetEffectBattlepassGrindPoints( bpReward.flav )
		if ( points )
			RuiSetString( btnRui, "itemCountString", string( points ) )
	}
...
void function BattlePass_Purchase( var button, int startQuantity )
{
	...
	else if ( GetPlayerBattlePassPurchasableLevels( ToEHI( GetUIPlayer() ), activeBattlePass ) > 0 )
	{
		...
		rpdcfg.maxPurchasableLevelsCallback = int function() : ( activeBattlePass ) {
			return GetPlayerBattlePassPurchasableLevels( ToEHI( GetUIPlayer() ), activeBattlePass )
		}

		rpdcfg.startingPurchaseLevelIdxCallback = int function() : ( activeBattlePass ) {
			return GetPlayerBattlePassLevel( GetUIPlayer(), activeBattlePass, false )
		}
...
void function UpdateBattlePassProgress( bool show, bool instant = false )
{
	...
	RuiSetGameTime( rui, "showBPProgressStartTime", instant ? -100.0 : Time() )
...
void function AboutBattlePass1Dialog_OnOpen()
{
	...
	if ( activeBattlePass == null )
	{
		...
		activeBattlePass = GetPlayerLastActiveBattlePass( ToEHI( GetUIPlayer() ) )
		if ( activeBattlePass == null )
			return
	}
...
void function UIToClient_ItemPresentation( SettingsAssetGUID itemFlavorGUID, int level, float scale, bool showLow, var loadscreenPreviewBox, bool shouldPlayAudioPreview, string sceneRefName )
{
	...
	ShowBattlepassItem( flav, level, scale, loadscreenPreviewBox, shouldPlayAudioPreview, showLow )
...
void function ShowBattlepassItem( ItemFlavor item, int level, float scale, var loadscreenPreviewBox, bool shouldPlayAudioPreview, bool showLow = false )
{
...
void function ShowBattlePassItem_WeaponSkin( ItemFlavor item, float scale )
{
	...
	float weaponItemScale = WeaponItemFlavor_GetBattlePassScale( weaponItem )
	...
	model.SetModelScale( scale * weaponItemScale * 0.75 )
	model.SetParent( mover )
...
void function ShowBattlePassItem_EmoteIcon( ItemFlavor item, float scale, bool showLow )
{
	...
	int dataInt = GetDataIntForEmoteIcon( GetLocalClientPlayer(), item )
	thread CreateClientSideEmoteIcon( model, ItemFlavor_GetGUID( item ), Time(), dataInt, true )
