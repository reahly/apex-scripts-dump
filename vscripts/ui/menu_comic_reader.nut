...
{
	if ( (Time() - file.s_openedTime) < PANEL_FADE_IN_BLOCK )
		return //
	...
	{
		if ( file.s_transitionQueue == QUEUE_CLEAR && (Time() - file.s_transitionStartTime) > PANEL_TRANSITION_BLOCK_TIME )
		{
	...
	if ( file.s_panelIndex + 1 >= file.s_panels.len() )
	{
		...
		thread function() : ()
		{
			...
			RuiSetGameTime( GetTopReaderRui().rui, "fadeOutStartTime", Time() )
...
void function InitPagesForMenuOpen()
{
	...
	diag.dialogue = Localize( "#COMIC_CREDITS" )
	diag.hideTail = true
	...
	file.pin_viewStartTime = Time()
	AddComicPinEvent( file.s_startPageIndex, file.s_startPanelIndex, -1, -1 )

	//
	RuiSetGameTime( firstReaderRui.rui, "fadeInStartTime", Time() )
...
void function UpdatePanelRui( int panelIndex, ReaderRui readerRui, bool isForward, bool isTransitionOut )
{
	...
	RuiSetGameTime( rui, "transitionStartTime", file.s_transition ? Time() : RUI_BADGAMETIME )
...
{
	Assert( panelIndex in file.pakHandles )
...
void function OnClickedToAdvance( bool isForward )
{
	...
	if ( wantPanelIndex != file.s_panelIndex )
	{
		...
		file.s_transition = true
		file.s_transitionStartTime = Time()
...
void function AddComicPinEvent( int pageIndex, int panelIndex, int previousPageIndex, int previousPanelIndex )
{
	...
	float timeToRead = Time() - file.pin_viewStartTime
