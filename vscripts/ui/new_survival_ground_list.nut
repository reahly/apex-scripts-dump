...
#if CLIENT || UI 
global function NEW_SurvivalGroundList_LevelInit
#endif

#if(UI)
global function NEW_InitSurvivalGroundList
global function NEW_OpenSurvivalGroundListMenu

global function ClientToUI_NEW_SurvivalGroundList_OpenQuickSwap
global function ClientToUI_NEW_SurvivalGroundList_RefreshQuickSwap
...
#if(CLIENT)
global function NEW_IsGroundListMenuOpen
global function NEW_UpdateSurvivalGroundList

global function UIToClient_NEW_SurvivalGroundListOpened
global function UIToClient_NEW_SurvivalGroundListClosed
global function UIToClient_NEW_SurvivalGroundList_UpdateQuickSwapItem
global function UIToClient_NEW_SurvivalGroundList_OnQuickSwapItemClick
...
#if(CLIENT)
global struct NEW_SurvivalGroundListUpdateParams
{
	entity        player
...
#if CLIENT || UI 
void function NEW_SurvivalGroundList_LevelInit()
{
	#if(UI)
...
#if(UI)
void function NEW_InitSurvivalGroundList( var menu )
{
	fileVM.menu = menu
...
{
	RunClientScript( "UIToClient_NEW_SurvivalGroundListOpened", fileVM.menu )
}
#endif
...
void function OnMenuNavBack()
{
	...
	{
		RunClientScript( "UIToClient_NEW_CloseQuickSwapIfOpen" )
		return
	}
...
{
	SetBlurEnabled( false )
	RunClientScript( "UIToClient_NEW_SurvivalGroundListClosed" )
}
#endif

#if(UI)
void function NEW_OpenSurvivalGroundListMenu()
{
	CloseAllMenus()
...
#if(CLIENT)
void function UIToClient_NEW_SurvivalGroundListOpened( var menu )
{
	...
	else
	{
		...
		else
		{
			...
			if ( ownerEHI != null )
			{
				...
				{
					headerOwnerText = Localize( "#DEATHBOX_OWNER_SUFFIX", GetPlayerName( ownerEHI ) )
...
#if(CLIENT)
void function UIToClient_NEW_SurvivalGroundListClosed()
{
...
#if(CLIENT)
void function NEW_UpdateSurvivalGroundList( NEW_SurvivalGroundListUpdateParams params )
{
	...
	foreach ( entity lootEnt in params.currentLootEnts )
	{
		...
		bool hasSpecialAmmo = (lootFlavor.lootType == eLootType.MAINWEAPON && lootFlavor.ammoType == "")
		...
		for ( ; otherLootEntIdx < entryData.lootEnts.len(); otherLootEntIdx++ )
		{
			...
			//
			entity lootEntVaultPanel      = GetVaultPanelForLoot( lootEnt )
			bool lootEntIsVaultItem       = (IsValid( lootEntVaultPanel ) && IsVaultPanelLocked( lootEntVaultPanel ))
			entity otherLootEntVaultPanel = GetVaultPanelForLoot( otherLootEnt )
			bool otherLootEntIsVaultItem  = (IsValid( otherLootEntVaultPanel ) && IsVaultPanelLocked( otherLootEntVaultPanel ))
			if ( lootEntIsVaultItem && !otherLootEntIsVaultItem )
				continue //
			else if ( !lootEntIsVaultItem && otherLootEntIsVaultItem )
				break //

			//
			entity LootEntBloodTTPanel      = GetBloodTTRewardPanelForLoot( lootEnt )
			bool lootEntIsBloodTTItem       = (IsValid( LootEntBloodTTPanel ) && IsBloodTTRewardPanelLocked( LootEntBloodTTPanel ))
			entity otherLootEntBloodTTPanel = GetBloodTTRewardPanelForLoot( otherLootEnt )
			bool otherLootEntIsBloodTTItem  = (IsValid( otherLootEntBloodTTPanel ) && IsBloodTTRewardPanelLocked( otherLootEntBloodTTPanel ))
			if ( lootEntIsBloodTTItem && !otherLootEntIsBloodTTItem )
				continue //
	...
	foreach ( DeathBoxEntryData entryData, void _ in entriesToUpdateSet )
	{
		...
		bool hasSpecialAmmo = (isMainWeapon && entryData.lootFlav.ammoType == "")
	...
	for ( int playerIdx = 0; playerIdx < BLACK_MARKET_UI_TEAMMATES_SUPPORTED; playerIdx++ )
	{
		...
		RuiSetString( widgetRui, format( "player%dName", playerIdx ), player != null ? GetPlayerName( ToEHI( player ) ) : "" )
...
void function UpdateItem( DeathBoxListPanelItem item )
{
	...
	bool hasSpecialAmmo = (isMainWeapon && lootFlavor.ammoType == "")
	...
	entity bestLootEnt = (entryData.lootEnts.len() > 0 && IsValid( entryData.lootEnts[0] ) ? entryData.lootEnts[0] : null)
	entryData.isUsable = entryData.totalCount > 0 && (IsValid( bestLootEnt ) || entryData.predictingNewEntToBeCreated)
	...
	if ( IsValid( bestLootEnt ) && entryData.isUsable )
	{
		if ( isBlackMarket )
		{
			entity vaultPanel = GetVaultPanelForLoot( bestLootEnt )
			if ( IsValid( vaultPanel ) && IsVaultPanelLocked( vaultPanel ) )
				entryData.isVaultItem = true
...
#if(CLIENT)
bool function NEW_IsGroundListMenuOpen()
{
...
void function PerformItemAction( DeathBoxListPanelItem item, bool isAltAction, bool fromExtendedUse, bool isFromQuickSwap, int ornull backpackSwapSlot )
{
	...
	{
		StartMenuExtendedUse( item.allocatedButton, fileLevel.holdToUseElem, 0.4, void function( bool success ) : (item, isAltAction, backpackSwapSlot) {
			if ( success )
	...
	else
	{
		...
		if ( isBlackMarket && entryData.isVaultItem )
		{
			wtfLootVaultConfirmDialogDoItFunc = doIt
			RunUIScript( "ClientToUI_LootVaultConfirmDialog_Open", deathBox.GetOwner() == player )
		}
		else if ( isBlackMarket && entryData.isBloodTTItem )
		{
			wtfLootVaultConfirmDialogDoItFunc = doIt
...
#if(UI)
void function ClientToUI_LootVaultConfirmDialog_Open( bool isBlackMarketOwner )
{
	ConfirmDialogData data
	data.headerText = Localize( "#LOBA_ULT_BLACK_MARKET_VAULT_CONFIRM_HEADER" )

	if ( isBlackMarketOwner )
	{
		data.messageText = Localize( "#LOBA_ULT_BLACK_MARKET_VAULT_CONFIRM_BODY" )

		data.resultCallback = void function( int result ) {
			if ( result == eDialogResult.YES )
				RunClientScript( "UIToClient_LootVaultConfirmDialog_DoIt" )
		}
		OpenConfirmDialogFromData( data )
	}
	else
	{
		data.messageText = Localize( "#LOBA_ULT_BLACK_MARKET_VAULT_DENY_BODY" )
...
#if(UI)
void function ClientToUI_BloodTTStealConfirmDialog_Open( bool isBlackMarketOwner )
{
	ConfirmDialogData data
	data.headerText = Localize( "#LOBA_ULT_BLACK_MARKET_BLOODTT_CONFIRM_HEADER" )

	if ( isBlackMarketOwner )
	{
		data.messageText = Localize( "#LOBA_ULT_BLACK_MARKET_BLOODTT_CONFIRM_BODY" )

		data.resultCallback = void function( int result ) {
			if ( result == eDialogResult.YES )
				RunClientScript( "UIToClient_BloodTTRewardChamberConfirmDialog_DoIt" )
		}
		OpenConfirmDialogFromData( data )
	}
	else
	{
		data.messageText = Localize( "#LOBA_ULT_BLACK_MARKET_BLOODTT_DENY_BODY" )
		OpenOKDialogFromData( data )
	}
}
#endif
...
void function OpenQuickSwap( var itemButton, DeathBoxEntryData entryData, bool isAltAction )
{
	...
	RunUIScript( "ClientToUI_NEW_SurvivalGroundList_OpenQuickSwap", itemButton )
}
#endif
...
{
	RunUIScript( "ClientToUI_NEW_SurvivalGroundList_CloseQuickSwap", fileLevel.currentQuickSwapItemButton )
...
#if(CLIENT)
void function UIToClient_NEW_CloseQuickSwapIfOpen()
{
	CloseQuickSwapIfOpen()
}
#endif
...
{
	RunUIScript( "ClientToUI_NEW_SurvivalGroundList_RefreshQuickSwap" )
}
#endif


#if(UI)
void function ClientToUI_NEW_SurvivalGroundList_RefreshQuickSwap()
{
...
#if(UI)
void function ClientToUI_NEW_SurvivalGroundList_OpenQuickSwap( var itemButton )
{
	GridPanel_Refresh( fileVM.quickSwapGrid )
...
#if(UI)
void function ClientToUI_NEW_SurvivalGroundList_CloseQuickSwap( var itemButton )
{
...
{
	Hud_ClearToolTipData( button )
	RunClientScript( "UIToClient_NEW_SurvivalGroundList_UpdateQuickSwapItem", button, TranslateBackpackGridPosition( index ) )
}
#endif


#if(CLIENT)
void function UIToClient_NEW_SurvivalGroundList_UpdateQuickSwapItem( var button, int backpackSwapSlot )
{
	entity viewPlayer = GetLocalViewPlayer()
...
{
	RunClientScript( "UIToClient_NEW_SurvivalGroundList_OnQuickSwapItemClick", button, TranslateBackpackGridPosition( index ), false )
}
#endif
...
{
	RunClientScript( "UIToClient_NEW_SurvivalGroundList_OnQuickSwapItemClick", button, TranslateBackpackGridPosition( index ), true )
}
#endif


#if(CLIENT)
void function UIToClient_NEW_SurvivalGroundList_OnQuickSwapItemClick( var button, int backpackSwapSlot, bool isRightClick )
{
	entity localPlayer = GetLocalClientPlayer()
...
void function OnMouseLeftPressed( var _ )
{
	if ( CanRunClientScript() )
		RunClientScript( "UIToClient_NEW_SurvivalGroundList_OnMouseLeftPressed" )
}
#endif


#if(CLIENT)
void function UIToClient_NEW_SurvivalGroundList_OnMouseLeftPressed()
{
