...
const array<string> EVENT_STRING_SEARCHTAGS_CHANGED =
[
	"#CLUB_EVENT_SEARCHTAGS_CHANGED_00"
]

const array<string> EVENT_STRINGS_CLUBTAG_CHANGED =
[
	"#CLUB_EVENT_CLUBTAG_CHANGED_00"
]
...

{
	...
	HudElem_SetRuiArg( file.clubInvitesButton, "buttonText", "#CLUB_INVITES_BUTTON" )
...
void function SetClubFooterOptions( var panel )
{
	...
	AddPanelFooterOption( panel, LEFT, BUTTON_X, true, "#LOBBY_CLUBS_LEAVE", "#LOBBY_CLUBS_LEAVE_PC", LeaveCurrentClub, CanUserLeaveClub )
	AddPanelFooterOption( panel, LEFT, BUTTON_Y, true, "#FIND_CLUBMEMBERS_CONSOLE", "#FIND_CLUBMEMBERS_PC", OpenFindClubMemberDialog, CanUserInviteClubMembers )
}
...
{
	if ( !file.tabsInitialized && ClubIsValid() )
	{
		//
...
void function RefreshDiscoverySearch( var button )
{
	...
	if ( button != null )
	{
		file.timeSinceLastDiscoverySearch = Time()
		//
...
void function RefreshCycleButtonDebounceThread()
{
	...
	while ( Time() - file.timeSinceLastDiscoverySearch < DISCOVERY_REFRESH_DEBOUNCE_SECONDS )
		wait( 1.0 )
...
var function ClubLanding_SearchResultButton_SetActionText( string actionString )
{
	...
	HudElem_SetRuiArg( file.selectedSearchResultButton, "actionSendTime", Time(), eRuiArgType.GAMETIME )
...
void function ProcessDeclinedInvites()
{
	foreach ( declinedClub in file.declinedClubs )
	{
		...
		file.declinedClubs.removebyvalue( declinedClub )
	}
}
...
{
	if ( ClubJoinRequestsCount() > 0 && ClubGetMyMemberRank() >= CLUB_RANK_CAPTAIN )
	{
		Hud_Show( file.joinRequestsPanel )
	}
	else
	{
		Hud_Hide( file.joinRequestsPanel )
	}
	Hud_SetText( file.joinRequestsLabel, Clubs_GetJoinRequestsString() )
}
...
void function UpdateClubLobbyMemberList()
{
	...
	foreach ( var button in file.memberButtons )
	{
		...
		Hud_RemoveEventHandler( button, UIE_CLICKRIGHT, MemberButton_OnRightClick )
	}
...
void function InitMemberButton( var button )
{
	...
	Hud_AddEventHandler( button, UIE_CLICKRIGHT, MemberButton_OnRightClick )
...
void function ConfigureMemberButtonToolTip( var button, ClubMember memberData, EadpPresenceData presence )
{
	...
	{
		if ( isOnline )
			toolTipData.actionHint3 = Localize( "#CLUB_MEMBER_BUTTON_TOOLTIP_INVITE" )
...
void function MemberButton_OnClick( var button )
{
	...
	if ( clubMember.memberID != GetLocalClientPlayer().GetPINNucleusId() )
	{
		...
		InviteClubMemberToParty( clubMember )
		MarkMemberAsInvited( button )
	}
}

void function InviteClubMemberToParty( ClubMember clubMember )
{
	int hardware = GetHardwareFromName( GetPlayerHardware() )
...
void function InviteAllClubMembersToParty()
{
	...
	{
		CommunityUserInfo ornull userInfoOrNull = GetUserInfo( GetNameFromeHardware( member.memberHardware ), member.memberID )
		if ( userInfoOrNull != null )
		{
			CommunityUserInfo userInfo = expect CommunityUserInfo( userInfoOrNull )
			if ( userInfo.isOnline && !userInfo.partyInMatch && !userInfo.partyFull )
				InviteClubMemberToParty( member )
		}
	}
}
...
void function InviteAllToPartyButton_OnClick( var button )
{
	...
	foreach ( var memberButton in file.memberButtons )
	{
		if ( memberButton in file.memberButtonToDataMap )
		{
			ClubMember clubMember = file.memberButtonToDataMap[ memberButton ]

			if ( !Clubs_IsUserAClubmate( clubMember.memberID ) )
			{
				file.memberListMapNeedsReset = true
				continue
			}
			...
			if ( clubMember.memberID != GetLocalClientPlayer().GetPINNucleusId() )
			{
				PIN_Club_Invite( ClubGetHeader(), "party", clubMember.memberID )
				var rui = Hud_GetRui( memberButton )
				MarkMemberAsInvited( memberButton )
			}
		}
	}
...
const float INVITE_ALL_BUTTON_DEBOUNCE_TIME = 3.0
void function InviteAllButtonDebounceThread()
{
	...
	while ( Time() < (startTime + INVITE_ALL_BUTTON_DEBOUNCE_TIME) )
	{
		wait( 0.5 )
	}
...
bool function MemberButton_OnKeyPress( var button, int keyId, bool isDown )
{
	...
	clubMemberToFriend.hardware = GetNameFromeHardware( clubMember.memberHardware )

	InspectFriend( clubMemberToFriend )
	return true
}
...
{
	array<ClubEvent> eventLog = ClubGetEventLog()
...
void function ConfigureClubEventButton( ClubEvent clubEvent, var eventButton, bool isNewEventRule = false )
{
	...
	if ( !isNewEventRule )
	{
		RuiSetBool( buttonRui, "isNewEventLine", false )
		string description
		switch( clubEvent.eventType )
		{
			...
			case CLUB_EVENT_RANK_CHANGE:
				description = Localize( GetStringForClubEvent( clubEvent ), clubEvent.eventText, Clubs_GetClubMemberRankString( clubEvent.eventParam ), Clubs_GetClubMemberNameFromNucleus(clubEvent.memberID) )
				//
				break;
...
{
	string eventString = clubEvent.eventText
	...
	string description = Localize( GetStringForClubEvent( clubEvent ), eventStrings[0], placementString, eventStrings[1] )
	//
...
string function GetStringForClubEvent( ClubEvent clubEvent )
{
	...
	if ( eventStrings.len() == 0 )
		return Localize( "#CLUB_EVENT_ERROR_MISSING" )
...
string function GetClubEventTimeString( int eventTime )
{
	//
	if ( !IsFullyConnected() )
		return Localize( "#CLUB_EVENT_TIMESTAMP_HOURS", 0, 0)

	int fakeTimeDays   = GetConVarInt( "test_fakeTimeDays" ) * SECONDS_PER_DAY
	...
	if ( displayTime.days > 0 )
		...
		timeString = Localize( "#CLUB_EVENT_TIMESTAMP_HOURS", displayTime.hours, displayTime.minutes )
...
void function ConfigureClubChatButton( ClubEvent chatEvent, var button, bool isNewEventRule = false )
{
	...
	{
		//
		string timeString = GetClubEventTimeString( chatEvent.eventTime )
		...
		RuiSetString( buttonRui, "chatString", chatEvent.eventText )
	}
}
...
int function AttemptToInsertNewEventLine( array<ClubEvent> eventLog, bool isForChat = false )
{
	...
	foreach ( event in eventLog )
	{
		if ( event.eventTime > lastViewedTime )
		{
			...
			printf( "ClubNewnessDebug: %s(): Event index %i is newer (%i) than most recent view time (%i)", FUNC_NAME(), ruleIndex, event.eventTime, lastViewedTime )
			break
		}
	}
	if ( ruleIndex > 0 )
...
int function ClubLobby_GetOnlineMemberCount( bool includeLocalUser = false )
{
	...
	foreach ( member in clubMembers )
	{
		...
		if ( presence.online )
		{
			if ( includeLocalUser || (!includeLocalUser && member.memberID != GetUIPlayer().GetPINNucleusId()) )
				onlineCount++
		}
	}
