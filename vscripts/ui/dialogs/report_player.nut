...
void function InitReportPlayerDialog( var newMenuArg ) //
{
	...
	HudElem_SetRuiArg( file.reportOtherButton, "buttonText", Localize( "#REPORT_CHAT" ) )
...
{
	string playerHardware = GetNameFromeHardware( playerHardwareID )
	...
	//
	#if CONSOLE_PROG && !NX_PROG 
		ver = IsUserOnSamePlatform( playerHardware ) ? minint( ver, 1 ) : ver //
	#endif
...
void function ReportOtherButton_OnActivate( var button )
{
	...
	#if CONSOLE_PROG && !NX_PROG 
		if ( IsUserOnSamePlatform( file.reportPlayerHardware ) )
		{
			//
			//
	...
	if ( file.friendlyOrEnemy == "friendly" || !IsUserOnSamePlatform( file.reportPlayerHardware ) || isSwitchPlatform )
	{
		//
