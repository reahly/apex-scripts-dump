...
void function InitSelectSlotDialog( var newMenuArg )
{
	...
	file.panels[ "quips" ] <- Hud_GetChild( menu, "SelectSlotQuips" )
...
void function __TryOpenSelectSlotDialog()
{
	...
	{
		if ( file.character != null )
		{
			if ( file.loadoutEntries[ 0 ] == Loadout_CharacterQuip( expect ItemFlavor( file.character ), 0 ) )
			{
				panelName = "quips"
			}
		}
