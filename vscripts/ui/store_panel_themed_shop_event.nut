...
void function ThemedShopPanel_UpdateGRXDependantElements()
{
	...
	if ( menuIsUsable )
	{
		...
		foreach ( int offerButtonIdx, var offerButton in file.offerButtons )
		{
			...
			Assert( offerData.prices.len() == 1 )

			file.offerButtonToOfferMap[offerButton] <- offerData
			bool shouldHighlight = (offerButtonIdx < 4)

			Hud_Show( offerButton )
			OfferButton_SetDisplay( offerButton, offerData, activeThemedShopEvent, shouldHighlight )
		}
	}
	else
	{
...
void function OfferButton_SetDisplay( var button, GRXScriptOffer offerData, ItemFlavor activeThemedShopEvent, bool shouldHighlight )
{
	...
	if ( GetConVarBool( "assetdownloads_enabled" ) )
		itemImage = GetDownloadedImageAsset( offerData.imageRef, offerData.imageRef, ePakType.DL_STORE, button )
	else
	...
	foreach( ItemFlavor flav in offerData.output.flavors )
	{
		...
		if( ItemFlavor_HasSourceTag( flav ) )
