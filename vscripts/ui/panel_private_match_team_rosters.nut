...
RosterStruct function CreateTeamRoster( int teamIndex, int teamSize, var panel )
{
	...
	Hud_AddEventHandler( teamRoster.headerPanel, UIE_CLICKRIGHT, OnRosterHeader_ClickRight )
	Hud_AddEventHandler( teamRoster.headerPanel, UIE_DOUBLECLICK, OnRosterHeader_DoubleClick )
...
void function PrivateMatch_TeamRosters_Update( table< int, array< entity > > teamPlayersMap )
{
	...
	foreach ( teamIndex, teamRoster in file.teamRosters )
	{
		...
		if ( teamIndex == TEAM_UNASSIGNED )
		{
			...
			PrivateMatch_TeamRoster_Configure( teamRoster, GameRules_GetTeamName( teamRoster.teamIndex ) )
		}

		string teamName = GameRules_GetTeamName( teamRoster.teamIndex )
		if ( teamRoster.teamIndex == 1 )
...
void function RosterButton_Init( var button, entity player = null )
{
	...
	if ( rosterButtonData.player != player )
	{
	...
	Hud_SetNew( button, player == null ? false : player.GetPlayerNetBool( "isReady" ) )
}


void function OnRosterButton_Click( var button )
{
	if ( InputIsButtonDown( MOUSE_LEFT ) && PrivateMatch_CanAssignPlayers( GetLocalClientPlayer() ) )
		thread TrackMouseDrag( button )
}


void function OnRosterHeader_ClickRight( var button )
{
	int teamIndex = -1
...
void function OnRosterHeader_DoubleClick( var button )
{
	foreach ( teamRoster in file.teamRosters )
	{
		if ( teamRoster.headerPanel != button )
